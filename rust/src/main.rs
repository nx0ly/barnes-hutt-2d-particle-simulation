...
fn main() {
    ...
    let mut control = Control2D::new(0.5, 100.);
    let mut bh_quad = BHQuadtree::new(0.5);
    ...
    let mut particle_mesh = Gm::new(
        InstancedMesh::new(&context, &Instances::default(), &CpuMesh::circle(12)),
...
fn update_simulation(bh_quad: &mut BHQuadtree, particles: &mut Vec<Particle>) {
    ...
    for p in particles {
        ...
        let total_force: Vec2 = bh_quad.accumulate(target, |wp: WeightedPoint| {
            ...
            let dir = target - wp.pos;
            let dist_sq = dir.length().powi(2) + 0.005;
            let strength = wp.mass / dist_sq;
            dir.normalize() * strength
        });

        // F = m*a
        // hah physics finally coming in clutch
        let accel = total_force / p.mass;
        p.vel += accel * 0.5;
        p.pos += p.vel * 0.5;
    }
}
